"""
Attempt 3 summary: reward combines PnL and inventory penalty r_t = dPnL_t - lambda|q_t|^2. Goal is to keep inventory near zero while still capturing spread; tuning lambda is delicate (too high collapses to no-trade, too low ignores inventory). In the tuned regime the agent oscillates around flat by posting one-sided quotes; bid-only and ask-only actions dominate, no-quote becomes rare. This hybrid policy beats the simple MM on PnL and volatility, and its midprice tracks the latent fundamental more closely, implying better liquidity and price efficiency.
"""
...
class RLMM3(BaseTrader):
    """
    Minimal tabular Q-learning market maker.

    - State: binned inventory and spread.
    - Action: no-quote, bid-only, ask-only, or symmetric quoting offsets (tick distances).
    - Reward: incremental PnL minus a quadratic inventory penalty: dPnL - inventory_penalty * inventory^2.
    ...
    def __init__(self, id, name, type, symbol, starting_cash,
        ...
        # Action space: (bid_offset, ask_offset); None means no quote on that side.
        default_actions = [
            (None, None),  # no-quote
            (5, None),     # bid-only
            (None, 5),     # ask-only
